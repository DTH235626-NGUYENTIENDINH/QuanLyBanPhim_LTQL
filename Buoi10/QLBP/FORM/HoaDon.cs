...
namespace QLBP.FORM
{
    public partial class frmHoaDon : Form
    {
        ...
        {
            using (frmHoaDon_ChiTiet chiTiet = new frmHoaDon_ChiTiet(0, false, currentNhanVien))
            {
                chiTiet.ShowDialog();
            }
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            using (frmHoaDon_ChiTiet chiTiet = new frmHoaDon_ChiTiet(id, false, currentNhanVien))
            {
                chiTiet.ShowDialog();
            }
            frmHoaDon_Load(sender, e);
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Xác nhận xoá hoá đơn này?", "Xoá", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                if (hd != null)
                {
                    ...
                    context.SaveChanges();
                    QLBP.SERVICES.LogService.LogInfo(currentNhanVien?.HoVaTen ?? "Hệ thống", "Xoá hoá đơn", $"Đã xoá hoá đơn số: {maHD}");
                }
                frmHoaDon_Load(sender, e);
            }
        }
        ...
        private void dataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if(dataGridView.Columns[e.ColumnIndex].Name == "ChiTiet" && e.RowIndex >= 0)
            {
                ...
                using (frmHoaDon_ChiTiet frm = new frmHoaDon_ChiTiet(maHoaDon, true, currentNhanVien))
                {
                    frm.ShowDialog();                   
                }
                frmHoaDon_Load(sender,e);   
            ...
            if (saveFileDialog.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    new DataColumn("Thành tiền", typeof(double))
                        ...
                        QLBP.SERVICES.LogService.LogInfo(currentNhanVien?.HoVaTen ?? "Hệ thống", "Xuất Excel hoá đơn", "Đã xuất báo cáo hoá đơn và chi tiết ra file Excel.");
        ...
        private void btnInHoaDon_Click(object sender, EventArgs e)
        {
            if (dataGridView.CurrentRow == null)
            {
                MessageBox.Show("Vui lòng chọn một hoá đơn để in!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            int maHoaDon = Convert.ToInt32(dataGridView.CurrentRow.Cells["ID"].Value.ToString());
        ...
        private void btnInPhieuBH_Click(object sender, EventArgs e)
        {
            if (dataGridView.CurrentRow == null)
            {
                MessageBox.Show("Vui lòng chọn một hoá đơn để in phiếu bảo hành!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            int maHoaDon = Convert.ToInt32(dataGridView.CurrentRow.Cells["ID"].Value.ToString());
            using (Reports.frmInPhieuBaoHanh inPhieuBH = new Reports.frmInPhieuBaoHanh(maHoaDon))
            {
                inPhieuBH.ShowDialog();
            }
        }

        private void dataGridView_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            // Kiểm tra nếu là cột "TongTien"
            if (dataGridView.Columns[e.ColumnIndex].Name == "TongTien")
            {
                // Cố định màu chữ là hồng/tím và in đậm để không bị AlternatingRows ghi đè
