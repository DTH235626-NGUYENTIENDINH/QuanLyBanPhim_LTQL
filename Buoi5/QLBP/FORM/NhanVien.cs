                    ...
                    using (XLWorkbook workbook = new XLWorkbook(openFileDialog.FileName))
                    {
                        ...
                        if (table.Rows.Count > 0)
                        {
                            foreach (DataRow r in table.Rows)
                            {
                                ...
                                nv.QuyenHan = r["Quyền hạn"].ToString().Trim().ToLower() == "quản lý" ? false : true;
                                context.NhanVien.Add(nv);
                            }
                            context.SaveChanges();
            ...
            if (saveFileDialog.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    new DataColumn("ID", typeof(int)),
                        ...
                        new DataColumn("Quyền hạn", typeof(string))
                    });
                    var nv = context.NhanVien.ToList();
                    if (nv != null)
                    {
                        foreach (var p in nv)
                            table.Rows.Add(p.ID, p.HoVaTen, p.DienThoai, p.DiaChi, p.TenDangNhap, p.MatKhau, p.QuyenHan ? "Nhân viên" : "Quản lý");
                    }
